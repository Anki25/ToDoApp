...
app.controller('TaskController', [
		...
		function($scope, TaskService, $location, $rootScope) {
			...
			self.updateByAdminTask= function(task) {
				console.log('updating task by admin TaskController');
				TaskService.updateByAdminTask(task).then(self.fetchAllTasks,
						function(errResponse) {
							console.error('Error while updating Tasks by admin');
						});
			};	
...
self.editbyadmin=function(){
	
	console.log('Updating Task by admin', self.task);
	
	self.updateByAdminTask(self.task);
...
/*GET SELECTED USER'S TASK DETAILS*/

self.getSelectedUserTask = getUserTask

function getUserTask(user_id) {
	console.log("getting task of user! " + user_id)
  TaskService.getUserTask(user_id).then(function(d) {
		self.tasks = d;					
	}, function(errResponse) {
		console.error('Error while fetching task details of user');
	});
};
