...
app.factory('TaskService',['$http','$q','$rootScope',function($http,$q,$rootScope){
	...
	var BASE_URL='http://localhost:1234/ToDoBackEnd'
		return{
...
updateByAdminTask:function(task){
	console.log('updating taskByAdmin TaskService');
	return $http.put(BASE_URL+'/tasksUpdateByAdmin/',task)
	.then(
			function(response){
				return response.data;
			},
			function(errResponse){
				console.error('Error while updating TaskByAdmin');
				return $q.reject(errResponse);
			}				
);
},
...
getUserTask: function(id){
	return $http.get(BASE_URL+'/tasksofuser/'+id)
	.then(                                        //success handler,failure handler
			function(response){
				$rootScope.selectedUserTask=response.data
				return response.data;
			},
			function(errResponse){
				console.error('Error while getting Task of users');
				return $q.reject(errResponse);
			}				
);
},
