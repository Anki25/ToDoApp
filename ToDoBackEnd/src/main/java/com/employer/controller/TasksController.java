...
public class TasksController {
	...
	@RequestMapping(value="/tasksofuser/{id}",method=RequestMethod.GET)
	public ResponseEntity<List<Tasks>> listAllTasksOfUser(@PathVariable("id") int user_id){
		List<Tasks> tasks=tasksDAO.taskOfUser(user_id);
		if(tasks.isEmpty()){
			return new ResponseEntity<List<Tasks>>(HttpStatus.NO_CONTENT);
		}
	...
	@RequestMapping(value="/tasksUpdateByAdmin/",method=RequestMethod.PUT)
	public ResponseEntity<Tasks> updateTaskByAdmin(@RequestBody Tasks tasks){
		
		if(tasksDAO.update(tasks)==true){
			tasks.setErrorCode("200");
			tasks.setErrorMessage("task updated");
		}
		return new ResponseEntity<Tasks>(tasks,HttpStatus.OK);
			}
